...
class Ambiente:
    ...
    def graficar_grilla(self):
        ...
        colores = ["white", "green", "red", "blue"]  # índice 0 = blanco, 1 = verde...
        cmap = ListedColormap(colores)

        fig, ax = plt.subplots(figsize=(5,5))
        cax = ax.imshow(self.grilla_visual, cmap=cmap, interpolation='none')

        # Se mejora la leyenda con colores explícitos
        legend_elements = [
            Patch(facecolor='green', label='1 = Activa no resistente'),
            Patch(facecolor='red', label='2 = Inactiva'),
            Patch(facecolor='blue', label='3 = Activa resistente'),
        ]
