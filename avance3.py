...
class Bacteria:
    ...
    def mutar(self):
        if self.__estado == "activa" and not self.__resistente:
            if random.random() < 0.05:  # 5% chance de volverse resistente
...
class Ambiente:
    ...
    def graficar_grilla(self):
        ...
        legend_elements = [
            Patch(facecolor=cmap(1/5), label='1 = Bacteria activa no resistente'),
            Patch(facecolor=cmap(2/5), label='2 = Bacteria inactiva'),
            Patch(facecolor=cmap(3/5), label='3 = Bacteria activa resistente'),
        ]
...
class MiAplicacion(Gtk.Application):
    ...
    def do_activate(self):
       ...
       # boton_bacterias.connect("clicked", self.on_)
    ...
    def on_graficar_click(self, button):
       ...
       # def on_agregar_biofilm(self, button):
